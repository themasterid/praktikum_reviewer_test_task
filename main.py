...
# это поможет нам разобратся с типами данных.
...
class Record:
    ...
    # через аннатацию типов.
    # # Так же для amount и comment.
    # В будущем это пригодится. И так по всему коду, повторно делать аанатацию
...
class Calculator:
    ...
    def get_today_stats(self):
        ...
        # Если работа скрипта затянится или скрипт отработает,
        # не в стыке смены дат можно получить не то значение.
        for Record in self.records:
            if Record.date == dt.datetime.now().date():
                today_stats = today_stats + Record.amount
        # Тут можно попробывать лучше, проитерироваться по всем записям
    ...
    def get_week_stats(self):
        week_stats = 0
        # Отличное рещение вынести дату в отдельную переменную!
...
class CashCalculator(Calculator):
    ...
    # не обезательно передавать в метод константы класса
    # USD_RATE, EURO_RATE, достаточно обратитmся к ним по self
    def get_today_cash_remained(self, currency,
        ...
        if cash_remained > 0:
            # тут локоничнее выглядит такая конструкция
